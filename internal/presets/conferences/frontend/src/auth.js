import { api, redirectByRole, setSession } from './api.js'
...
export function initLoginForm(formId, alertId) {
...
export function initRegisterForm(formId) {
  const form = document.getElementById(formId)
  form?.addEventListener('submit', async (e) => {
    e.preventDefault()
    ...
    if (!payload.email || !payload.email.includes('@')) {
      setHint('email-hint', 'Укажите корректный e-mail')
      ok = false
    }
    if (payload.login.length < 6 || !loginRe.test(payload.login)) {
      setHint('login-hint', 'Логин: минимум 6 символов, только латиница и цифры')
      ok = false
    }
